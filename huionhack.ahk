...
; *** if  the exe is 64 bit then the AHK script must be 64 bit***
;msgbox % "Base Address: " mem.BaseAddress

SetFormat, IntegerFast, d 

; these offsets were found with cheat engine , i toggle through my monitors  with value 0, 1 , ....
pointerHSBat := [0x000398D8 + mem.BaseAddress, "UInt", 0xD0, 0x88, 0xF0, 0x70, 0x1A8] 
...
actualMonitorMapping := mem.read(pointerHSBat*) ;  monitor the huion driver maps to


; this is the toggle funtion
if (actualMonitorMapping + 1 == monitorsCount ){ ; if last monitor switch to first one 
    ...
    xxx := actualMonitorMapping + 1
    mem.write(0x000398D8 + mem.BaseAddress, xxx, "UInt", 0xD0, 0x88, 0xF0, 0x70, 0x1A8) ; monitor + 1
}

Gui, Show
Gui, Destroy ; TODO: dummy Gui, ugly hack to update the process , dont know anything better for now 
